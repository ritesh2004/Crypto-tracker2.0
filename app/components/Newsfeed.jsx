...
function Newsfeed() {
  ...
  return (
    ...
    <span style={{fontFamily:'Oxygen',color:'black',fontWeight:'700',fontSize:'1.5rem'}}><i class="bi bi-square-fill" style={{color:'blueviolet'}}></i> NewsFeed</span>
      {news && news.length>0 ?
        news.map((feeds,id)=>{
          const dateString = new Date(feeds.published_on)
          ...
          return (
            <div className='w-[95%] h-[150px] shadow-lg rounded-lg flex flex-row bg-white overflow-hidden p-3' style={{fontFamily:'Oxygen'}}>
            <div className='flex flex-col gap-3'>
              <span className='text-sm' style={{fontFamily:'Oxygen',fontWeight:'300',color:'black',letterSpacing:'0.01719rem'}}>{day}{month} {hour}:{minute}</span>
